...
def Q_1(cursor, conn, execution_time):
    ...
    query = """ SELECT player_name, AVG(xgoals) as xgoal_avg
...
def Q_3(cursor, conn, execution_time):
    ...
    query = """SELECT player_name, COUNT(*) as first_timers
    FROM shots INNER JOIN 
    (
        SELECT match_id FROM competitions INNER JOIN matches ON competitions.season_id = matches.season_id 
...
def Q_4(cursor, conn, execution_time):
    ...
    FROM passes INNER JOIN 
    (
        SELECT match_id FROM competitions INNER JOIN matches ON competitions.season_id = matches.season_id WHERE competitions.comp_name = 'La Liga' AND competitions.season_name = '2020/2021'
    ) USING (match_id) 
...
def Q_5(cursor, conn, execution_time):
    ...
    query = """ SELECT p_name, count 
    FROM players INNER JOIN 
    (
        SELECT recipient, COUNT(*) FROM passes 
        WHERE match_id IN 
        (
            SELECT match_id 
            FROM competitions INNER JOIN matches ON competitions.season_id = matches.season_id 
            WHERE competitions.comp_name = 'Premier League' AND competitions.season_name = '2003/2004'
        ) GROUP BY recipient
    ) AS counts 
    ON counts.recipient = players.player_id ORDER BY count DESC;"""
...
def Q_6(cursor, conn, execution_time):
    ...
    WHERE match_id IN 
    (
        SELECT match_id FROM competitions INNER JOIN matches ON competitions.season_id = matches.season_id WHERE competitions.comp_name = 'Premier League' AND competitions.season_name = '2003/2004'
    ) 
    GROUP BY team_name ORDER BY COUNT(*) DESC; """
...
def Q_7(cursor, conn, execution_time):
    ...
    query = """SELECT player_name, COUNT(*) AS throughball_count 
    FROM passes 
...
def Q_8(cursor, conn, execution_time):
    ...
    query = """ SELECT team_name, COUNT(*) AS throughball_count 
    FROM passes 
    WHERE technique = 'Through Ball' AND match_id IN (
        SELECT match_id FROM competitions INNER JOIN matches ON competitions.season_id = matches.season_id 
        WHERE competitions.comp_name = 'La Liga' AND competitions.season_name = '2018/2019' OR competitions.season_name = '2019/2020' OR competitions.season_name = '2020/2021'
...
def Q_9(cursor, conn, execution_time):
    ...
    FROM dribbles 
    WHERE complete AND match_id IN 
    (
        SELECT match_id FROM competitions INNER JOIN matches ON competitions.season_id = matches.season_id WHERE competitions.comp_name = 'La Liga' AND competitions.season_name = '2018/2019' OR competitions.season_name = '2019/2020' OR competitions.season_name = '2020/2021'
    ) 
    GROUP BY player_name ORDER BY COUNT(*) DESC;"""
...
def Q_10(cursor, conn, execution_time):
    ...
    query = """ SELECT player_name, COUNT(*) as dribbled_pasts 
    FROM dribbles_past WHERE match_id in 
    (
        SELECT match_id FROM competitions INNER JOIN matches ON competitions.season_id = matches.season_id WHERE competitions.comp_name = 'La Liga' AND competitions.season_name = '2020/2021'
    ) 
    GROUP BY player_name ORDER BY COUNT(*) ASC;"""
...
AND endloc_z >= 1.33 AND endloc_z <= 2.67 
AND ((endloc_y >= 36 AND endloc_y <= 38.67) OR (endloc_y >= 41.33 AND endloc_y <= 44)) 
GROUP BY player_name ORDER BY COUNT(*) DESC; """
...
(
    SELECT match_id 
    FROM competitions INNER JOIN matches ON competitions.season_id = matches.season_id WHERE competitions.comp_name = 'La Liga' AND competitions.season_name = '2020/2021'
)
AND endloc_x >= 102 AND endloc_x <= 120 AND endloc_y >= 18 AND endloc_y <= 62
AND outcome IS NULL
GROUP BY team_name ORDER BY COUNT(*) DESC;"""
